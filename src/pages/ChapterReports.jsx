...
import { ChevronDown, ChevronUp, Eye, Edit3, Trash2, Calendar, Filter, CheckCircle2, X, Upload } from 'lucide-react';

const ChapterReports = () => {
  const [isDropdownOpen, setIsDropdownOpen] = useState(false);
  const [activeModal, setActiveModal] = useState(null); // 'monthly' | 'annual' | null
  const [reportView, setReportView] = useState('monthly'); // 'monthly' | 'annual'
      ...
      <div className="bg-white rounded-[24px] p-6 shadow-[0_8px_30px_rgba(0,0,0,0.02)] border border-[#eeeeee] flex flex-wrap gap-4 items-center">
        ...
        <div className="relative min-w-[200px]" ref={dropdownRef}>
          <button 
            ...
            className="w-full flex items-center justify-between bg-[#f4f4f5] border border-transparent hover:border-[#eeeeee] rounded-xl py-2.5 px-5 text-[13px] font-normal text-[#1a1a1a] outline-none transition-all cursor-pointer"
          >
            <span>Month</span>
            {isDropdownOpen ? <ChevronUp size={16} className="text-[#555555] opacity-40" /> : <ChevronDown size={16} className="text-[#555555] opacity-40" />}
          </button>
          ...
          {isDropdownOpen && (
            <div className="absolute top-full left-0 right-0 mt-2 bg-white rounded-xl shadow-xl border border-gray-100 py-2 z-[60] animate-in fade-in zoom-in-95 duration-200">
              <button 
                onClick={() => { setActiveModal('monthly'); setReportView('monthly'); setIsDropdownOpen(false); }}
              ...
              <button 
                onClick={() => { setActiveModal('annual'); setReportView('annual'); setIsDropdownOpen(false); }}
        ...
        <div className="flex items-center justify-between flex-wrap gap-6">
          <div className="flex flex-col gap-1">
            <h4 className="text-[18px] font-bold text-[#1a1a1a]">
              {reportView === 'monthly' ? 'Monthly' : 'Annual'} completion progress bar
            </h4>
